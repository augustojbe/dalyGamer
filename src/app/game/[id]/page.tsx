...
export async function generateMetadata({params}: PropsParams): Promise<Metadata>{
    try{
        ...
        const response= await fetch(`${process.env.NEXT_API_URL}/next-api/?api=game&id=${id}`, {next: {revalidate: 20}})

        if (!response) {
            throw new Error('Erro ao buscar jogo')
        }

        const data1: GameProps = await response.json();

        return {
            title: `${data1.title} | DalyGames`,
            description: `${data1.description.slice(0,100)}...`,
            openGraph:{
                title: data1.title,
                images: [data1.image_url]
            },
            robots:{
                index: true,
                follow: true,
...
async function getGameSorted(){
    try{
        ...
        throw new Error("Faiiled to fetch data")
    }
}
